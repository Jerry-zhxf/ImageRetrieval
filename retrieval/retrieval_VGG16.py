...
def extract_feature_query(model, img, use_gpu=True):
    ...
    outputs = model(input_img)
    # print(type(outputs))
    _, predicted = outputs.topk(2, 1, largest=True, sorted=True)
    # _, predicted = torch.max(outputs.data, 1)  # 返回最大值和其索引
    # predicted = outputs.numpy().tolist()
    # sorted(range(len(predicted)), key=lambda k: predicted[k], reverse = True)
    # print(outputs.shape)
    # print(outputs)
    # print(predicted)
    predicted = predicted.cpu().data.tolist()
    # print(predicted)
    ff = outputs.data.cpu()
    fnorm = torch.norm(ff,p=2,dim=1, keepdim=True)
    # print(fnorm)
...
if __name__ == '__main__':
    ...
    # Prepare model.
    model = load_model(pretrained_model=os.path.join('./DaiNet/checkpoint', 'DaiNet', 'ckpt.t7'),
                        use_gpu=True)
