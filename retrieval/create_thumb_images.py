...
def create_thumb_images(full_folder, thumb_folder, suffix='thumb', height=100, del_former_thumb=False):
    ...
    count = 0
    for dir_path in os.listdir(full_folder):
        for image_file in os.listdir(full_folder+dir_path):
            try:
                #print(dir_path, image_file)
                image = cv2.imread(full_folder + dir_path + '/' + image_file)
                ...
                if not os.path.isdir(thumb_folder + dir_path):
                    os.makedirs(thumb_folder + dir_path)
                cv2.imwrite(thumb_folder + dir_path + '/' + image_name + suffix + image_extension, resized_image)
            except Exception as e:
                count += 1
                os.remove(full_folder + dir_path + '/' + image_file)
                print(count, full_folder + dir_path + '/' + image_file, e)
                # print(e)
