...
netname = 'DealNet'
writer_train = SummaryWriter(comment='DealNet_train')  # 提供一个 comment 参数，将使用 runs/日期时间-comment 路径来保存日志
writer_test = SummaryWriter(comment='DealNet_test')
# writer_grpah = SummaryWriter(comment='DealNet_grpah')
# writer_grpah.add_graph(DealNet(),torch.rand(1, 3, 32, 32))
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        torch.save(state, './checkpoint/' + netname + '/DealNet_300epoch.t7')
        best_acc = acc
