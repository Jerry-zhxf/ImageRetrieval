...
netname = 'VGG16'
writer_train = SummaryWriter(comment='VGG16_train')  # 提供一个 comment 参数，将使用 runs/日期时间-comment 路径来保存日志
writer_test = SummaryWriter(comment='VGG16_test')
...
if args.resume:
    ...
    checkpoint = torch.load(os.path.join('./checkpoint', netname, 'ckpt1.t7'))
...
optimizer = optim.Adam(net.parameters(), lr=args.lr, betas=(0.9, 0.99), weight_decay=5e-4) #betas 用于计算梯度以及梯度平方的运行平均值的系数, weight_decay (float, 可选) – 权重衰减
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        torch.save(state, './checkpoint/' + netname + '/VGG16_100epoch.t7')
        best_acc = acc
