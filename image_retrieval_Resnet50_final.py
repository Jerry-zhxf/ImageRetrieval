...
from retrieval.retrieval_Resnet50 import load_model, load_data, extract_feature, load_query_image, sort_img, extract_feature_query
...
def dynamic_modification_timer(mutex):
    ...
    for path in os.listdir(update_path):
        data_loader = load_data(data_path=update_path+path+'/',
                                batch_size=1,
                                shuffle=False,
                                transform='default',
                                )
        ...
        if len(increase_image_paths) != 0:
            ...
            gallery_feature = np.load('./retrieval/models/'+path+'_feature.npy')
            gallery_feature = torch.from_numpy(gallery_feature)
    ...
    for path in os.listdir(delete_path):
        data_loader = load_data(data_path=delete_path + path + '/',
                                batch_size=1,
                                shuffle=False,
                                transform='default',
                                )
...
# Prepare model. 加载预训练的model
model = load_model(pretrained_model=os.path.join('./DealNet/checkpoint', 'ResNet50', 'ResNet50_50epoch.t7'), use_gpu=True)
print("Model load successfully!")
...
def image_retrieval():
    ...
    if request.method == 'POST':
        if request.form['submit'] == 'upload':
            ...
            query_feature, label = extract_feature_query(model=model, img=query_image)  # [1,2048]
            print(label)
            gallery_feature = np.load('./retrieval/models/'+label+'_feature.npy')
            gallery_feature = torch.from_numpy(gallery_feature)
            image_paths = np.load('./retrieval/models/'+label+'_paths.npy')
            image_paths = image_paths.tolist()
